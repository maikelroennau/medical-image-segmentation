    ...
    def save_labelme(self, file_names, dirpath, save_json_only=True):
        ...
        for file in file_names:
            ...
            if not save_json_only:
                ...
                subprocess.call(['cp', os.path.join(self.imgpath, filename), dirpath])


if __name__ == "__main__":
    if len(sys.argv) > 3:
        ds = CocoDatasetHandler(sys.argv[1], sys.argv[2])
        ds.coco2labelme()
        ds.save_labelme(ds.labelme.keys(), sys.argv[3])
    else:
        print("Please provide the images directory path, the ouput directory and the annotations JSON file")
