...
def evaluate(model, images_path, batch_size, input_shape=None, loss_function=None, classes=1, one_hot_encoded=False):
    if loss_function:
        loss = loss_function
    else:
    ...
    if Path(model).is_file():
        ...
        loaded_model.compile(optimizer=Adam(learning_rate=1e-5), loss=loss, metrics=[METRICS])
