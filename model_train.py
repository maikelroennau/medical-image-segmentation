...
def load_images_and_masks(path, batch_size=16, target_size=(1920, 2560), seed=2149, augment=False, save_to_dir=None):
    supported_types = [".tif", ".tiff", ".png", ".jpg", ".jpeg"]
    images_paths = [image_path for image_path in Path(path).joinpath("images").glob("*.*") if image_path.suffix.lower() in supported_types]
    masks_paths = [mask_path for mask_path in Path(path).joinpath("masks").glob("*.*") if mask_path.suffix.lower() in supported_types]
    ...
    if augment:
        ...
        train_data = images_datagen.flow(images, masks, batch_size=batch_size, seed=seed, shuffle=False, save_prefix="augmented_", save_to_dir=save_to_dir)
        return train_data
    else:
        images = images * (1./255.)
        masks = masks * (1./255.)
...
train_data = load_images_and_masks("dataset/train/", batch_size=batch_size, target_size=(height, width), augment=True)
validation_data = load_images_and_masks("dataset/train/", batch_size=batch_size, target_size=(height, width))
...
history = model.fit(train_data, validation_data=validation_data, validation_steps=len(validation_data[0]), batch_size=batch_size, epochs=epochs, steps_per_epoch=steps_per_epoch, callbacks=callbacks) #, initial_epoch=10), validation_data=val_ds)
